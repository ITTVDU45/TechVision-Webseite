...
interface WebDevelopmentInsightsProps {
  posts: BlogPost[];
}

export default function WebDevelopmentInsights({ posts }: WebDevelopmentInsightsProps) {
  // Convert posts to match BlogPost format from data/blogPosts.ts
  const convertedPosts = posts.map((post, index) => ({
    id: post.id || post.link?.replace('/blog/', '') || `web-dev-${index}`,
    title: post.title,
  ...
  return (
    <section id="web-development-insights" className="py-24 bg-black relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="max-w-4xl mx-auto text-center mb-16">
          ...
          >
            <span className="text-sm font-medium text-blue-400">Web Development & Insights</span>
          </motion.div>
          ...
          >
            Web Development <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-blue-500 to-indigo-600">Insights</span>
          </motion.h2>
          ...
          >
            Aktuelle Trends und Best Practices in der Webentwicklung
          </motion.p>
        </div>
