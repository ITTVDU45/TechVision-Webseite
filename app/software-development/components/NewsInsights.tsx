...
export default function NewsInsights({
  title = "Neuigkeiten & Insights aus der Softwareentwicklung",
  description = "Erfahren Sie mehr über aktuelle Trends, Best Practices und Erfolgsgeschichten aus unseren Projekten.",
  blogPosts = [],
}: NewsInsightsProps) {
      ...
      <section className="py-32 bg-black relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="text-center py-12">
            <h3 className="text-2xl font-semibold text-white">Keine Blog-Artikel verfügbar</h3>
    ...
    <section className="py-32 bg-black relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="relative max-w-7xl mx-auto">
              ...
              {visiblePosts.map((post, index) => (
                ...
                >
                  <Link href={post.link || "#"} className="group block">
                    <div className={`bg-gradient-to-br p-[1px] rounded-2xl h-full overflow-hidden`}>
                      <div className="bg-gray-900 p-4 rounded-2xl h-full flex flex-col overflow-hidden">
                        ...
                        <button className="w-full py-2 px-4 bg-gradient-to-r from-blue-500 via-indigo-500 to-violet-600 rounded-xl text-white text-sm font-medium hover:shadow-lg transition-all duration-300 relative overflow-hidden">
                          Weiterlesen
                        </button>
                      </div>
                    </div>
          ...
          >
            <ChevronRightIcon
              className={`w-6 h-6 ${page >= pageCount - 1 ? "text-blue-400/50" : "text-blue-400"}`}
            />
          </button>
            ...
            {Array.from({ length: pageCount }).map((_, idx) => (
              <button
                ...
                className={`w-2 h-2 rounded-full transition-all duration-300 ${
                  page === idx ? "bg-blue-500 w-8" : "bg-blue-500/20 hover:bg-blue-500/40"
                }`}
