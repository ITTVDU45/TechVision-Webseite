...
interface InfrastructureNewsSectionProps {
  posts: BlogPost[];
}

export default function InfrastructureNewsSection({ posts }: InfrastructureNewsSectionProps) {
  // Convert posts to match BlogPost format from data/blogPosts.ts
  const convertedPosts = posts.map((post, index) => ({
    id: post.id || post.link?.replace('/blog/', '') || `it-infra-${index}`,
    title: post.title,
  ...
  return (
    <section id="infrastructure-news" className="py-24 bg-black relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="max-w-4xl mx-auto text-center mb-16">
          ...
          >
            <span className="text-sm font-medium text-blue-400">IT-Infrastruktur & News</span>
          </motion.div>
          ...
          >
            Aktuelle <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-blue-500 to-indigo-600">IT-Infrastruktur News</span>
          </motion.h2>
